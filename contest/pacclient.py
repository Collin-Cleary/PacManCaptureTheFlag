...
class PacClient(asyncore.dispatcher):
  ...
  def startgame(self,code,line):
    ...
    agentFactory = self.agentFactoryBuilder(not self.isBlue)
    self.agents = [agentFactory.getAgent(agentIndexBase+2*i) for i in range(3)]
...
def readCommand( argv ):
  ...
  parser.add_option('-a', '--agents', help=default('Agent factory for your team'),
                    default='OffenseDefenseAgents')
  parser.add_option('--agentArgs', help=default('Arguments passed to the factory'),
                    default='')  
  ...
  agentArgs = parseAgentArgs(options.agentArgs)
  args['agentFactoryBuilder'] = loadAgentFactoryBuilder(options.agents, agentArgs)
  ...
  if options.quiet:
    if options.agents.startswith('Keyboard'):
...
def loadAgentFactoryBuilder(factory, args):
  "Returns a function that takes isRed and returns an agent factory"
  ...
  for moduleDir in pythonPathDirs:
    ...
    for modulename in moduleNames:
      ...
      if factory in dir(module):
        factoryClass = getattr(module, factory)
        return lambda isRed: factoryClass(isRed=isRed, **args)
