...
def readCommand( argv ):
  ...
  parser.add_option('-r', '--red', help=default('Agent factory for red team'),
                    default='KeyboardAgents')
  parser.add_option('-b', '--blue', help=default('Agent factory for blue team'),
                    default='AllOffenseAgents')
  parser.add_option('--redOpts', help=default('Options for agent factory for red team'),
                    default='')
  parser.add_option('--blueOpts', help=default('Options for agent factory for blue team'),
                    default='')
...
def loadAgents(isRed, factory, textgraphics, args):
  ...
  for moduleDir in pythonPathDirs:
    ...
    for modulename in moduleNames:
      try:
        ...
        continue
      if factory in dir(module):
        if textgraphics and factory.startswith('Keyboard'):
          raise Exception('Using the keyboard requires graphics (no text display, quiet or training games)')
        foundFactory = getattr(module, factory)(isRed=isRed, **args)
        indexAddend = 0
        if not isRed: indexAddend = 1
        indices = [2*i + indexAddend for i in range(3)]
        return [foundFactory.getAgent(i) for i in indices]
        
  raise Exception('The agent factory ' + factory + ' is not specified in any *Agents.py.')

def replayGame( layout, agents, actions, display, length ):
    rules = CaptureRules()
    game = rules.newGame( layout, agents, display, length )   
    state = game.state
...
def runGames( layout, agents, display, length, numGames, record, numTraining ):
  ...
  for i in range( numGames ):
    ...
    if beQuiet:
        ...
        gameDisplay = display
        rules.quiet = False    
    g = rules.newGame( layout, agents, gameDisplay, length )              
    g.run()
